...
int main(int argc, char **argv) {
	d3t12::MicroControllerCommandPort portu(d3t12::MicroControllerCommandPort::OStreamPtr(new std::ofstream("/dev/ttyACM0")));
	d3t12::MicroControllerLEDCommandBuilder ledCommandbuilder;

	portu << ledCommandbuilder.setLED(0).setPower(true).build();
	sleep(2);
	portu << ledCommandbuilder.setLED(1).setPower(true).build();
	sleep(2);
